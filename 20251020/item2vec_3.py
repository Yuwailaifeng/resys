...
for file_hour in file_hour_list:
    ...
    sql_list.append("""
        SELECT `device_uuid`, GROUP_CONCAT(DISTINCT CONCAT(CAST(`content_type` AS STRING), "|", CAST(`content_id_f` AS STRING)), ";") content_id_sequence 
        FROM `dw_v2`.`user_ls_day_summary` 
        WHERE `ds` = '""" + str(file_hour) + """' AND `content_type` = 3 AND `device_uuid` IS NOT NULL AND `content_id_f` IS NOT NULL 
...
try:
    with connection.cursor() as cursor:
        ...
        sql = """
            SELECT DISTINCT t1.content_id, t1.page_name, t2.content_name_f, t2.content_id_c, t1.content_type, t1.channel_id
            FROM dm.recommend_waterfall_v2_content_list t1
            LEFT JOIN dw_v2.user_ls_day_summary t2
            ON t1.content_id = t2.content_id_f
            WHERE t1.ds = '""" + str(file_hour_list[-1]) + """' AND t1.content_type = '回听'
                AND t2.ds = '""" + str(file_hour_list[-1]) + """' AND t2.content_type = 3 AND t2.content_name_f IS NOT NULL 
        """
        print(sql)
        ...
        print("content_column_for_recs", file_hour_list[-1], len(tempDf), columns)
        tempDf.to_csv("./model_data/" + str(file_hour_list[-1]) + ".content_column_for_recs.txt", header=columns, index=False, sep="\t", encoding="UTF-8")
...
page_name_dict = {}
channel_id_dict = {}
with open("./model_data/" + str(file_hour_list[-1]) + ".content_column_for_recs.txt", encoding="UTF-8") as file:
    for line in file.readlines():
        ...
        id_name_dict[line[0]] = line[3] + "|" + "#".join(page_name_dict[line[0]]) + "|" + line[2] + "|" + "#".join(channel_id_dict[line[0]])
...
def i2i_save_file(similarity_score_list, key_list, id_name_dict, vectors_similarity_file, vectors_similarity_name_file):
    ...
    print("i2i_save_file", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def u2i_save_file(user_content_id_reco_dict, id_name_dict, vectors_similarity_file, vectors_similarity_name_file, user_his_dict):
    ...
    print("u2i_save_file", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    with open(vectors_similarity_file, "w", encoding="UTF-8") as file:
    ...
    with open(vectors_similarity_name_file, "w", encoding="UTF-8") as file:
        for key, value in user_content_id_reco_dict.items():
            ...
            res_name = [k + "|" + id_name_dict[k] for k in value[:100]]
            file.write(key + "\n" + " ; ".join(his_res_name) + "\n" + " ; ".join(res_name) + "\n\n")
    print("u2i_save_file", datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
for key, value in user_his_dict.items():
    ...
    for idx in range(len(content_id_similarity_dict) - 1):
        if len(user_content_id_reco_dict[key]) >= 3000:
            break
        for item in value:
...
#
# 03  09 * * * source /etc/profile && pyenv global 3.7.9 && source ~/.bashrc && cd /data/azx_reco/yt_recall/item2vec_3 && /home/ubuntu/.pyenv/versions/3.7.9/bin/python3 -u ./item2vec_3.py 1>>./log.txt 2>&1

os.system("ls -lah ./impala_data")
os.system("ls -lah ./sample_data")
os.system("ls -lah ./model_data")

hdfs_file_path = "/user/admin/an_zhong_xin/yt_recall/item2vec_3/" + str(content_hour_list[-1] + "/")
