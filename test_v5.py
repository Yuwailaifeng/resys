...
for idx in range(len(dnn_dims)):
    if idx == 0:
        w_list.append(tf.Variable(tf.random_normal([
            net.get_shape().as_list()[1],
            dnn_dims[idx]
        ], seed=1), name="weight_" + str(idx)))
        b_list.append(tf.Variable(tf.zeros([
            dnn_dims[idx]
        ]), name="bias_" + str(idx)))
    else:
        w_list.append(tf.Variable(tf.random_normal([
...
with tf.Session(config=config) as sess:
    ...
    plt.plot(X_train_data, sess.run(pred, feed_dict={
        input["X"]: X_train_data
    }))
    ...
    plt.plot(X_test, sess.run(pred, feed_dict={
        input["X"]: X_test
    }))
...
# all_album.play_count.sort(reverse=True)[:200]
# all_album.play_time.sort(reverse=True)[:200]

# history_album = filter(
#     lambda x: x.label == "历史", all_album
# )
