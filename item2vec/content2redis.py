...
with open(item2vec_recall_file_list[0], encoding="UTF-8") as file:
    for line in file.readlines():
        try:
            ...
            reco_content_id_list = [item for item in line[1].split(";")]

            album_reco_dict.setdefault(count // batch_size, {})
            album_reco_dict[count // batch_size][device_uuid + "_album_item2vec"] = ";".join(reco_content_id_list[:top_k])
        except:
            print(line)
...
with open(item2vec_recall_file_list[3], encoding="UTF-8") as file:
    for line in file.readlines():
        try:
            ...
            single_reco_dict[count // batch_size][device_uuid + "_single_item2vec"] = ";".join(content_id_list[:top_k])
        except:
            print(line)
...
# 00  05 * * * source /etc/profile && pyenv global 3.7.9 && source ~/.bashrc && cd /data/azx_reco/yt_recall && /home/ubuntu/.pyenv/versions/3.7.9/bin/python3 -u ./content2redis.py 1>>./log.txt 2>&1
